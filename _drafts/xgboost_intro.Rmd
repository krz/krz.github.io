...
We still have `r nrow(data)` observations left. The variable we want to predict is `income` which can take two states, _small_ and _large_, so we are facing a binary classification problem.
...
We already get a pretty good AUC value of `r unlist(auc@y.values)`, here is the plot:
...
To do this, we split the training data set again, in a new (smaller) training set and a validation set. Before, we had `r nrow(train)` observations in our training set. We now use the first 2000 observations from this set for early stopping:
...
We now get an AUC value of `r unlist(auc@y.values)`. This is not an improvement, it is even slightly worse than our prediction from above. The reason for this may be because
...
Let's start from the beginning, otherwise our code will soon become very ugly. `idx` contains all indices of the test data. We split `train` in 10 parts (10 folds), so each validation part has `r floor(nrow(train)/10)` observations. This is easy with the `createFolds()` function in the `caret` package:
